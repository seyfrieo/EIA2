...
namespace Aufgabe9 {
    ...
    function init(_event: Event): void {
        for (let i: number = 0; i < word.length; i++) {
            ...
            tempLetter.addEventListener("mousedown", selectLetter); //Event Listener, damit Buchstaben ausgewählt werden können
        ...
        //Buchstaben in Canvas eingefügt
    ...
    function handleKeyDown(_event: KeyboardEvent): void {
        for (let i: number = 0; i < letter.length; i++) { 
            if (String.fromCharCode(_event.keyCode) == letter[i].innerHTML) { //Schaut in ASCII-Tabelle nach ausgewähltem Buchstaben
                if (selectedLetter != undefined) //Farbe wird zurückgesetzt
                    selectedLetter.style.backgroundColor = "#FBF8EF";
                selectedLetter = letter[i]; //Buchstabe wird gefunden und überschrieben
    ...
    function placeLetter(_event: MouseEvent): void {
        if (_event.altKey == true)
            return; // Unterbricht Funktion, wenn ALT gedrückt
    ...
    //Was ich anklicke, wird gelöscht
